...
int main()
{
    ...
    vertexShader1 = glCreateShader(GL_VERTEX_SHADER); //创建一个着色器 并且用ID进行引用
    glShaderSource(vertexShader1, 1, &vertexShaderSource1, NULL); //附加到着色器对象上 编译硬编码
    glCompileShader(vertexShader1);

    vertexShader2 = glCreateShader(GL_VERTEX_SHADER); //创建一个着色器 并且用ID进行引用
    glShaderSource(vertexShader2, 1, &vertexShaderSource2, NULL); //附加到着色器对象上 编译硬编码
    ...
    //检测着色器构建情况
    check_ShaderBuild(vertexShader1);
    check_ShaderBuild(vertexShader2);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 更新uniform颜色 实际上就是更新着色器中的输入
        glUniform4f(vertexColorLocation, colorValue, colorValue, 0.5f, 1.0f);
        //绘制三角形
        glBindVertexArray(VAOs[0]);
